...
public class StockTest {
  ...
  public void testViewGainLoss() throws IOException {
    // Test for GOOG
    double gainLoss = Stock.viewGainLoss("GOOG", "2020-03-04", "2020-03-09", api, library);
    assertEquals(4.76, gainLoss, 0.01); // ((1100 - 1050) / 1050) * 100

    // Test for AAPL
    gainLoss = Stock.viewGainLoss("AAPL", "2020-03-04", "2020-03-09", api, library);
  ...
  public void testViewXDayMovingAverage() throws Exception {
    // Test for GOOG
    double movingAverage = Stock.viewXDayMovingAverage("GOOG", "2020-03-04", 2, api, library);
    assertEquals(1075.0, movingAverage, 0.01); // (1050 + 1100) / 2

    // Test for AAPL
    movingAverage = Stock.viewXDayMovingAverage("AAPL", "2020-03-04", 2, api, library);
  ...
  public void testViewXDayMovingAverageWithInsufficientData() {
    try {
      ...
      assertEquals("Number of days goes too far back, or we could not find an average for the date provided.", e.getMessage());
    } catch (Exception e) {
  ...
  public void testViewXDayCrossOver() throws Exception {
    // Test for GOOG
    List<String> crossovers = Stock.viewXDayCrossOver("GOOG", "2020-03-04", 2, api, library);
    assertEquals(1, crossovers.size());
    assertTrue(crossovers.contains("2020-03-09 (Bullish Crossover)"));

    // Test for AAPL
    crossovers = Stock.viewXDayCrossOver("AAPL", "2020-03-04", 2, api, library);
    assertEquals(1, crossovers.size());
    assertTrue(crossovers.contains("2020-03-09 (Bullish Crossover)"));
  }
  ...
  public void testViewXDayCrossOverWithInsufficientData() {
    try {
      ...
      assertEquals("Insufficient data points to calculate the 3-day moving average.", e.getMessage());
    } catch (Exception e) {
      fail("Unexpected Exception thrown: " + e.getMessage());
    }
  }
  ...
  public void testGetters() {
    Stock stock = new Stock("2020-03-04", 1000, 1100, 950, 1050, 10000);
